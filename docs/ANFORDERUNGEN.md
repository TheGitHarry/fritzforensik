...
| H10 | Der Report weist die **selbst verursachten Spuren** aus: Der Abzug meldet sich an der Box an, und die Box protokolliert das wie jede andere Anmeldung. Die Einträge des Ereignisprotokolls, die im Fenster der Sicherung liegen, sind markiert, und der Umstand ist benannt. Verglichen wird in der Ortszeit der Box; ist deren Zeitzone unbestimmbar, wird nicht markiert, sondern gesagt, dass nicht markiert werden konnte | erfüllt | `test_ereignisse_im_sicherungszeitraum_werden_markiert`, `test_report_weist_die_eigenen_spuren_im_ereignisprotokoll_aus`, `test_fenster_beginnt_beim_start_des_abzugs_nicht_beim_ersten_abruf`, `test_ereignis_auf_der_fenstergrenze_zaehlt_dazu`, `test_sicherungsfenster_auch_ohne_boxtime_datenart`, `test_ohne_bekannte_zeitzone_wird_nicht_markiert_sondern_gesagt` |
| H11 | Der Report weist je Gerät den Zeitpunkt des **ersten und letzten Kontakts** aus, soweit die Box ihn führt. Eine fehlende Angabe wird als fehlend kenntlich gemacht und nie als „nie gesehen“ dargestellt | erfüllt | `test_hosts_tragen_zeitangaben_aus_query_lua`, `test_host_ohne_zeitangabe_bleibt_leer_und_nicht_none`, `test_hosts_sektion_zeigt_beide_zeitspalten`, `test_hinweis_deutet_leere_zeitzellen_als_nicht_gefuehrt`, `test_hinweis_sagt_es_deutlich_wenn_die_box_gar_nichts_fuehrt` |
