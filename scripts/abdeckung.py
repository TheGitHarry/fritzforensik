...
def erzeuge(befunde: list[dict]) -> str:
    ...
    a("sehen, was Ihr Beitrag abdeckt.")
    a("")
    a("**Einreichen** als Issue über die Vorlage *Abdeckung melden*")
    a("([neues Issue anlegen](https://github.com/TheGitHarry/fritzforensik/issues/new?template=abdeckung.yml)).")
    a("Den Inhalt von `auszug.json` einfach einfügen. Das Issue ist öffentlich — deshalb")
    a("ist der Auszug so gebaut, dass daran nichts Schützenswertes ist; lesen Sie ihn")
    a("vorher trotzdem durch. Auf dieser Seite werden eingegangene Auszüge per")
    ...
    a("muss — für die reine Abdeckung nie. Es gehört dann aber nicht in ein Issue,")
    a("sondern auf den Weg aus [SECURITY.md](SECURITY.md).")
    a("")
